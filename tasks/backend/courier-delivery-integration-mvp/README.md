...
**Обновлено**: 2025-10-29 (добавлена задача 11: SMS подтверждение)
**Версия**: MVP 1.2
...
### Задача 11: SMS подтверждение доставки

**Файл**: `11-add-sms-photo-required-fields.md`

**Что добавлено**:
- ✅ Добавление полей `isSmsRequired` и `isPhotoRequired` в API response
- ✅ Поддержка SMS подтверждения доставки
- ✅ Доступ к endpoint обновления статуса для роли DRIVER
- ✅ Интеграция мобильного приложения готова и ждет этих изменений

**Изменения** (всего 3 простых правки):
1. `CargoLoadingResponse.java` - добавить 2 поля
2. `CustomerMapper.java` - добавить 2 поля в маппер
3. `CourierWaybillController.java` - добавить роль DRIVER

**Время**: 45 минут (разработка + тестирование)
