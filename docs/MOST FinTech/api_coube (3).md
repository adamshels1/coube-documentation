...
- `limit_amount` может быть `null` — клиент ещё на стадии скоринга или проверки.
- `factoring_available` — `true` (факторинг доступен), `false` (недоступен) или `null` (не определено).
- При отсутствии клиента в БД MOST — `404 Not Found`.
